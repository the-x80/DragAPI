...
namespace DragAPI {
	...
	public:
		...
		Vector(T comps[N]) {
			if (std::is_trivially_copyable<T>().value == true) {
		...
		Vector(Vector& other) {
			...
			//Developer note:
			//This could be simplified. Use one for loop with a ternary operator in the second parameter.
			if (this->ComponentCount() > other.ComponentCount()) {
				for (int i = 0; i < other.ComponentCount(); i++) {
		...
		float Dot(Vector<T, N> right) {
			return Vector::Dot(*this, right);
		}
		...
		static Vector Lerp(Vector left, Vector right, float value) {
			return Vector();
		}
