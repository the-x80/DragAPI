...
DragAPI::Quaternion::Quaternion()
{
	x = y = z = w = 0.0f;
}

DragAPI::Quaternion::Quaternion(float _w, float _x, float _y, float _z)
{
	this->x = _x;
	this->y = _y;
	this->z = _z;
...
DragAPI::Vector3 DragAPI::Quaternion::ToEulerAngles()
{
...
void DragAPI::Quaternion::FromEulerAngles(DragAPI::Vector3 angles)
{
	//Be sure to switch the axes if necessary.
	//Code was yoinked online. Might not work as intended
